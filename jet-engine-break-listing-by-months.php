...
class Jet_Engine_Break_Listing_By_Months {
	...
	public function setup() {
		...
		if ( ! defined( 'JET_ENGINE_BREAK_MONTH_OPEN_HTML' ) ) {
			...
			define( 'JET_ENGINE_BREAK_MONTH_OPEN_HTML', '<h4 class="jet-engine-break-listing" style="width:100%; flex: 0 0 100%; text-align:left;">' );
		}
	...
	//	if ( JET_ENGINE_BREAK_BY_FIELD ) {
			$date = get_post_meta( $post->ID, 'datums' , true );
	//	} else {
	//		$date = $post->post_date;
	//	}
	...
	public function render_month( $post ) {
		...
		if ( ! $timestamp ) {
			return;
                ...
                $cssclass = date_i18n( 'mY', $timestamp );

                echo "<div id='".$cssclass."' class='onepage'>";
